...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        ParComponent comp = null;
        while ((comp = nodesLoaded.poll()) != null) {
            if (comp.lock.tryLock()) {
                try {
                    if (!comp.isDead) {
                        Edge<ParComponent> minEdge = comp.getMinEdge();
                        if (minEdge == null) {
                            solution.setSolution(comp);
                            break;
                        }
                        else {
                            ParComponent other = minEdge.getOther(comp);
                            if (other.lock.tryLock() && !other.isDead) {
                                try {
                                    comp.merge(other, minEdge.weight());
                                    nodesLoaded.add(comp);
                                }
                                finally {
                                    other.lock.unlock();
                                }
